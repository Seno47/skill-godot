...
- For fixed/high-angle 3D districts, arenas, settlements, or extraction routes, pass builder-owned district-composition, local environment-integrity, and whole-map coverage contracts. Resolve final transforms; prove full-footprint occupancy and topmost surface/object relationships; partition the playable footprint into single-family zones with fallback/adjacency budgets; tile-survey every cell through the shipping camera; compare every enabled static collider and hero-radius blocked sample to visible render mass; and prove resolved production occluder aliases fade/restore. Legal origins, one permissive ground region, curated captures, complete collision/navmesh coverage, synthetic camera fixtures, or attractive stills cannot certify the map.
...
- Complete fixed/high-angle 3D district or arena: add rubric modifier `high-angle-3d-district-complete`; fill the district and environment review templates; run both `scripts/environment_integrity_audit.py` and `scripts/environment_coverage_audit.py` from resolved target-scene contracts; preserve raw target-build boundary/corridor/gameplay/repetition/contact close-ups, a zero-gap shipping-camera survey contact sheet, collider-shell/raster overlays, and production occluder/camera restoration video
