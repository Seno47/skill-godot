...
- Enemies, NPCs, perception, behavior/state logic, navigation, crowds, telegraphs, or off-screen simulation: [references/game-ai-and-navigation.md](references/game-ai-and-navigation.md)
...
- Complete fixed/high-angle 3D district or arena: add rubric modifier `high-angle-3d-district-complete`; fill the district/environment reviews and resolved-scene provenance manifest; run `scripts/resolved_scene_provenance_audit.py` with both evidence contracts, then `scripts/environment_integrity_audit.py` and `scripts/environment_coverage_audit.py`; preserve raw target-build boundary/corridor/gameplay/repetition/contact close-ups, a zero-gap shipping-camera survey contact sheet, collider-shell/raster overlays, and production occluder/camera restoration video
