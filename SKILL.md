...
- For fixed/high-angle 3D districts, arenas, settlements, or extraction routes, pass builder-owned district-composition, local environment-integrity, and whole-map coverage contracts. Bind all reports to one deterministic resolved dependency-closure digest covering the root scene, recursive/runtime resources, evidence exporters, project settings and selected export preset; a root `.tscn` hash is not a candidate revision. Resolve final transforms; prove full-footprint occupancy and topmost surface/object relationships; partition the playable footprint into single-family zones with fallback/adjacency budgets; tile-survey every cell through the shipping camera; compare every enabled static collider and hero-radius blocked sample to visible render mass; and prove resolved production occluder aliases fade/restore. Legal origins, one permissive ground region, curated captures, complete collision/navmesh coverage, synthetic camera fixtures, attractive stills, or root-only provenance cannot certify the map.
