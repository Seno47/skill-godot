...
| 42 детерминированные Python-утилиты | Аудиты проекта/ассетов, проверки transformed prop/surface/ground/clearance, visual checks, genre-aware difficulty и system contracts, crash/commerce/cloud/safety/upgrade/fault/hardware/assistive-пробы, композиция rubric, capture, бюджеты, scorecard и build size |
...
- **Fixed/high-angle 3D-районы:** видимые городские/ландшафтные границы, massing кварталов, landmarks/view corridors, функциональные story-зоны, модульное/material-направление, full-transform occupancy объектов, semantic ownership опорных поверхностей, sampling render-ground швов, overhead clearance и измеримая камера.
...
Для fixed/high-angle 3D-районов добавлен компонуемый modifier `high-angle-3d-district-complete` и руководство [`high-angle-3d-districts.md`](./references/high-angle-3d-districts.md). Его district-review отклоняет забор по периметру, clone filler, бессмысленный scatter, коридоры взгляда в пустоту и случайную плоскую перекраску фасадов. Отдельный контракт [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) и [`environment_integrity_audit.py`](./scripts/environment_integrity_audit.py) разрешают финальные Godot transforms, полные prop/support footprints, видимые ground triangles и vertical clearance, поэтому полный collision coverage больше не может скрыть пересечения, nature-пропы на дороге или швы с открытым substrate.
