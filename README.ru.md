...
Для fixed/high-angle 3D-районов используется компонуемый modifier `high-angle-3d-district-complete`. Layered workflow [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) связывает каждый geometry/semantic report с рекурсивным ResourceLoader/runtime dependency-closure digest и hash проекта/exporter/preset, затем запускает локальный geometry/strict-contact audit, whole-map coverage audit и schema-v2 [`visible_first_boundary_audit.py`](./scripts/visible_first_boundary_audit.py): порядок контактов по всему периметру плюс реальный whole-body escape тест production-капсулой. Если на карте есть улицы, [`road-and-streetscape-semantics.md`](./references/road-and-streetscape-semantics.md) добавляет schema-v5 resolved-scene проверки topology, road-end policy, exporter-resolved topmost render surfaces, marking/closure intersections, class-specific placement, facade roles, support contacts, authored incident closures и полного shipping-camera survey.
