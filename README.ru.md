...
Для fixed/high-angle 3D-районов используется компонуемый modifier `high-angle-3d-district-complete`. Layered workflow [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) связывает каждый geometry/semantic report с рекурсивным ResourceLoader/runtime dependency-closure digest и hash проекта/exporter/preset, затем запускает локальный geometry/strict-contact audit, whole-map coverage audit и [`visible_first_boundary_audit.py`](./scripts/visible_first_boundary_audit.py) для порядка контактов по всему периметру. Если на карте есть улицы, [`road-and-streetscape-semantics.md`](./references/road-and-streetscape-semantics.md) добавляет fail-closed resolved-scene аудит lane/junction/sidewalk/crosswalk/marking topology, непрерывности ширины sidewalk/curb-return у каждой стороны подхода и T-перекрёстка, приоритета crosswalk над drain/repair деталями, полных building/vehicle/furniture footprints, видимых facade/roof/trim material slots, approach-aware мебели, authored incident closures, достижимых карманов у невидимых safety walls и полного shipping-camera junction survey.
