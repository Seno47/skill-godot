...
- **Fixed/high-angle 3D-районы:** dependency-closure идентичность кандидата, композиция района, full-transform контакты, per-zone surface topology, zero-gap shipping-camera survey, all-static-collider/render-shell hero raster, exporter-owned flood fill реальной production-капсулой, continuity видимых ограничителей, whole-perimeter visible-first safety-wall clearance, явная road-end/topmost-surface политика, production occluder aliases, clearance и измеримая камера.
...
- **AI и генерация:** честное восприятие, навигация/replan/crowd recovery, память провалившейся стороны и bounded alternate/escalation, capacity evidence, именованные random streams, разрешимые seed cohorts, распределения, fallback и save/resume parity.
...
Для fixed/high-angle 3D-районов используется компонуемый modifier `high-angle-3d-district-complete`. Layered workflow [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) связывает каждый geometry/semantic report с рекурсивным ResourceLoader/runtime dependency-closure digest и hash проекта/exporter/preset, затем запускает локальный geometry/strict-contact audit, schema-v2 bidirectional visible-solid/collider coverage с явной non-solid VFX-классификацией и schema-v3 [`visible_first_boundary_audit.py`](./scripts/visible_first_boundary_audit.py): порядок контактов и whole-body escape production-капсулой, различение surface/solid collision intent, global-transform/contact-distance parity, non-climb traversal и проверка стыков модулей. Если на карте есть улицы, [`road-and-streetscape-semantics.md`](./references/road-and-streetscape-semantics.md) добавляет schema-v6 resolved-scene проверки topology, road-end policy, exporter-resolved topmost render surfaces, marking/closure intersections, class-specific placement, source-owned facade roles в паре противоположных диагональных MSAA-normalized masks, support contacts, authored incident closures и полного shipping-camera survey.

Evidence-код теперь явно считается только toolchain input: exporter transient-подключает project adapter после инстанцирования точного production root, provenance-аудит запрещает QA/report paths в production dependency closure, а [`build_size_audit.py`](./scripts/build_size_audit.py) сканирует exact PCK/embedded executable/ZIP на forbidden resource markers. Поэтому одного export exclude filter недостаточно.
...
Для всех законченных игр и production-срезов с видимым движением теперь действует общий builder-owned контракт [`production-motion-review.template.md`](./assets/production-motion-review.template.md) и руководство [`motion-and-animation.md`](./references/motion-and-animation.md). Они требуют раннюю motion-ячейку в игровом масштабе и normal-speed доказательства точного кандидата: отклик на основное действие, полный системный цикл, перемещение/поворот/контакт или передачу, плотные одновременные циклы и восстановление после прерывания. Наличие Tween, меняющиеся координаты, замедленная запись или выбранные позы больше не позволяют принять скольжение, одинаковый bounce, невесомые старты/остановки, промахи контактов, синхронно-роботическую автоматизацию или награду, не совпадающую с причиной.
...
Для `scripts/image_compare.py` и `scripts/mjpeg_avi_watchback.py` дополнительно нужен Pillow. Если его нет, соответствующие тесты корректно пропускаются, а утилиты явно сообщают о зависимости вместо ослабления visual-проверки.
