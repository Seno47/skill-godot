...
The scene-graph audit checks serialized hierarchy, parents, resource IDs/paths, signal endpoints, selected NodePaths, and common missing visual/collision properties. Its warnings about runtime-assigned values and methods require judgment. It cannot see nodes that were never saved, imported-scene internals, or runtime composition; engine import and play remain authoritative. A local node named by `[editable path]` is valid without an exception. A declaration, override, NodePath, or connection endpoint that is absent locally but lies below a statically proven `ExtResource` `PackedScene` boundary is recorded as `editable_packed_scene_internal_references`, not an unconditional missing-parent error. The boundary may be a named child instance or the current root when the scene root itself is instanced/inherited. Require a clean engine load/import of the exact wrapper before accepting that limitation. Missing parents and unknown editable paths outside every PackedScene boundary remain blocking.
