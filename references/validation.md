...
- Complete fixed/high-angle 3D district work has a builder-owned visible-boundary ledger, district/zone/view-corridor plan, modular-variation/repetition audit, target-build state matrix, normal-speed camera evidence, zero-error local `environment_integrity_audit.py`, and zero-error whole-map `environment_coverage_audit.py` backed by a complete shipping-camera survey, semantic zone/fallback/adjacency results, all-enabled collider/render-shell hero raster, production occluder aliases and topmost surface/object contacts.
