...
- for fixed/high-angle 3D districts, raw target-build entry/landmark, typical block, reachable boundary contact, long-corridor termination, dense interaction, objective/extraction, overview/max-zoom and visible-repetition-overlay frames; transformed prop-occupancy, semantic surface-ownership, render-ground seam and vertical-clearance close-ups; a deterministic full-footprint shipping-camera contact sheet; collider/render-shell and hero-radius overlays; topmost surface/object contacts; when streets exist, source-bound facade/roof/openings/trim masks, marking-mesh endpoint overlays, every typed lane-boundary termination close-up and every vertex-resolved canopy/awning/support contact; plus normal-speed production-occluder/follow/look-ahead/zoom/camera-volume/restoration video;
...
- high-angle architectural material direction: compare raw same-zone clusters, cross-zone transitions and gameplay-size facade detail under the exact shipping `WorldEnvironment`, lights, fog and tone mapping. Require semantic assignment by function/construction/age/story/adjacency, declared dominant/support/accent screen-area bands, preserved texture/PBR and facade/roof/trim structure, and physically/story-caused weathering. Reject random instance hue cycling, rainbow/checkerboard neighbors, flat opaque flood fills, accent-everywhere distribution, isolated swatches, 100% coverage and hue-count evidence;
- high-angle streetscape evidence integrity: compare the exporter-owned visible-mesh traversal with adapter classifications; reject invented semantic slots/area percentages, omitted lamps/poles/supports, resource-only palette probes, road endpoints without a resolved visual termination, endpoints inside facades and support structures without measured ground/mount contact. Inspect the raw target-build pixels and their resolved surface-ID masks, not only JSON totals;
...
- high-angle environment integrity: compare resolved full transformed prop volumes, full support-footprint surface samples, actual visible ground-mesh samples and overhead clearance with raw target-build close-ups. Collision coverage, legal origins, a broad floor collider or an overview image cannot pass penetrated facades/vehicles/towers, fused props, nature footprints on road/sidewalk, or exposed substrate seams;
- high-angle whole-map coverage: inspect per-zone top-surface families, fallback ratios and every observed adjacency/transition; verify the raw shipping-camera tile manifest has zero uncovered playable cells; compare every enabled static collider/hero-radius blocked sample with visible render mass; use exact production collision-root aliases and fade/restoration traces; and review forbidden topmost surface/object pairs. A permissive whole-map region, curated states, known-prop collision list or synthetic occluder fixture is a FAIL even at 100% reported sample coverage;
...
For a fixed/high-angle 3D district/arena case, use `assets/high-angle-3d-district-review.template.md` plus `assets/environment-integrity-review.template.md` as builder-owned precursors. When streets/intersections/sidewalks/furniture exist, add `assets/streetscape-semantics-review.template.md`. These must fail routine boundary, massing, repetition, semantic-density, camera-motion, transformed-contact, surface-ownership, visible-ground, road/junction/marking topology, facade-slot completeness, furniture placement, hidden-wall pockets and clearance defects before independent or owner review.
