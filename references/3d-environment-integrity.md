...
Schema v3 has three independent layers: exporter-owned production-body reachability, ordered visible-first contact, and resolved collision-assembly intent/parity. Migrate v2 by declaring every boundary assembly, binding its visible members and enabled shapes bidirectionally, tagging every approach class, adding independently measured render-contact distances, adding module-seam coverage, and adding production-body motion trials for solid-volume assemblies. Then regenerate through the copied probe; changing only `schema_version` is not a migration.

The adapter still declares the perimeter spans and ordered first-hit queries, but it may not declare the reachability grid or final assembly counts, shape classes and global-transform measurements. Author exactly one production-scene `production_boundary_reachability_region` with the required grid/query metadata, group the real hero body, enabled capsule shape and production starts as described in the exporter header, and let the exporter query the resolved `World3D` itself. The report must contain every grid cell, a nonempty unsafe fringe on the declared sides, the scene-owned safe polygon, actual ground and blocker masks, and an eight-neighbor fail-closed flood fill from the production starts. Grid cell size may not exceed the project ratio of the production hero radius. Hand-written `free_cells`, `blocked_cells`, `outside_cells`, a hard-coded rectangular boundary, an empty outside set or a grid too coarse to see a hero-width passage is not evidence.

### Separate surface parity from solid-volume parity

Choose collision intent before choosing the shape:

- `surface_parity` follows a complex static surface whose facets may legitimately support or guide the body. Exact/concave mesh collision can be appropriate for a static facade, cliff or structure with authored recesses and openings. Concave collision on a moving body is invalid, and one coarse convex hull must not close a real arch or doorway.
- `solid_volume_parity` represents an obstacle the player must not enter or climb: rubble heaps, piled vehicles, barricade masses or collapsed assemblies. Bind each visible member to one or more convex/primitive solid shapes and every shape back to visible mass. A concave triangle skin is not a solid: it can turn sloped debris into a ramp and leave passages between bodies even when collision coverage is numerically complete.
- a safety backstop is secondary containment only. It may catch pathological escape behind the authored boundary, but it cannot substitute for collision on the visible object and cannot be the first reachable contact anywhere. If a broad backstop can stop the production capsule before the visible edge, remove/retreat it or replace it with correctly bound visible solids.

The probe resolves the final production scene and overwrites adapter claims for visible member count, enabled collider-shape count/type and root transform parity. Compare the final **global** origin and basis of each visual asset root and its collision root; matching local transforms under different parents is insufficient. Exact member/shape counts plus bidirectional bindings prevent one broad hidden collider from standing in for many visible vehicles or debris pieces. Do not require a universal one-shape-per-mesh ratio: require complete, intentional bindings and the appropriate collision intent.

In the transient adapter declaration, each assembly supplies stable `visible_root_path`, `collision_root_path`, `visible_member_paths`, `collider_shape_paths`, `shape_bindings`, intent, composite kind, expected counts, required approach classes and transform tolerances. The copied Godot probe resolves those paths from the instantiated production root, removes the declaration-only member/shape lists, and writes `resolved_visible_member_paths`, actual enabled shape records and measured `global_transform_parity`. Missing/hidden nodes, disabled/null shapes, stale counts, one-way bindings or a non-static concave owner fail before the audit can pass.

For each mapped first hit, measure both physics contact distance and the corresponding render-shell contact distance using resolved mesh/bounds geometry. Their difference must remain below an authored final-size stand-off budget. Mapping an oversized collider to a visible object by name does not make an early invisible stop visible.

Declare every enabled perimeter span, not just road corridors or the places already visible in curated captures. Deterministically sample each span from endpoint to endpoint with spacing no greater than the production hero diameter, point each query inward-to-outward, and preserve exact ordered hits. A production-size capsule sweep is preferred. A ray implementation passes only as a full-width bundle whose lateral gaps are no larger than the declared hero-radius budget; one center ray cannot represent a character body. Tag and exercise edges, both corners, every relevant concavity, every adjacent-module seam and every authored opening negative. A central approach does not cover a facade recess or a local hole at a module joint. The exporter-owned whole-body flood fill must independently prove that the actual production capsule reaches zero unsafe-fringe cells while expected in-map free cells remain connected.

For every `solid_volume_parity` assembly, drive the actual production `CharacterBody3D` into each required approach under ordinary movement rules and record an uncut trace. The body must stop at the visible mass, never enter the declared occupied volume, and remain below the small project-authored elevation-gain tolerance. A static `intersect_shape()` result cannot prove that `move_and_slide()` will not climb a sloped triangle or thread between adjacent hulls.

Declare adjacent modular joints explicitly. Each seam needs at least one production-capsule sample tagged `module_seam`, the resolved visible members and collider shapes on both sides, and a target-build close-up. Review both directions: collision continuity with a visible crack still fails art integrity, while a visually continuous facade with a hero-width collision hole fails traversal integrity. Preserve opening negatives separately so sealing every joint cannot close doors, gates or arches.
...
10. the whole-perimeter visible-first contact sheet, exporter-owned production-capsule grid/contact sheet, collider-versus-render contact-distance rows and exact edge/corner/concavity/opening close-ups;
11. resolved collision-assembly intent, member/shape bindings, global-transform parity, solid-volume motion trials and every modular-seam close-up;
12. the visible-limiter baseline/current continuity ledger and replacement close-ups where applicable;
13. every strict intentional-contact pair, including the undeformed-clear or authored-deformation interface state.
14. representative non-solid fire/smoke/particle classes in the same target-build visual state, with their exclusion from physics recorded.
