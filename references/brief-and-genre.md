...
Treat crash resilience, commerce/entitlements, account/cloud cross-progression, online safety, upgrade compatibility, fault injection, desktop hardware coverage and assistive accessibility as conditional release modifiers. Select only what the brief promises or the actual architecture makes material; once selected, its gates are fail-closed and cannot be replaced by a library/SDK being present, a local mock, one development machine, or a future manual checklist.
...
If a complete game's challenge, tension, execution, cognition, risk, coordination, or competitive skill changes materially over time, define its difficulty envelope before multiplying encounters or levels. Read [difficulty-and-pacing.md](difficulty-and-pacing.md), adapt `assets/difficulty-pacing-contract.template.json`, and declare the genre profile, curve shape, difficulty dimensions, taught/combined skills, peaks/relief or player-selected routes, failure recovery, and adaptation policy. Add the `difficulty-pacing-complete` rubric modifier when the selected base case does not already include its gates. Do not assume every game needs the same sawtooth, and do not let a single rising number stand in for pacing.
