...
- **Fixed/high-angle 3D districts:** dependency-closure candidate identity, district composition, full-transform contacts with measured strict exemption plausibility, per-zone surface topology, approach/T-side sidewalk-curb band continuity, crosswalk priority over road details, zero-gap shipping-camera surveys, all-static-collider/render-shell hero rasters, exporter-owned production-capsule escape flood fill, visible-limiter continuity, whole-perimeter visible-first safety-wall clearance, explicit road-end/topmost-surface policy, production occluder aliases, overhead clearance, and measured camera motion.
...
- **AI and generation:** perception fairness, navigation/replan/crowd recovery, failed-side memory with bounded alternate/escalation, capacity evidence, named random streams, solvable seed cohorts, distributions, fallback, and save/resume parity.
...
Fixed/high-angle 3D districts use the composable `high-angle-3d-district-complete` modifier and [`high-angle-3d-districts.md`](./references/high-angle-3d-districts.md). Its district review fails clone filler and incoherent composition. The layered [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) workflow binds every geometry/semantic report to a recursive ResourceLoader/runtime dependency-closure digest with project, exporter and preset hashes, then runs local geometry, strict contact-exemption, schema-v2 bidirectional visible-solid/collider coverage with explicit non-solid VFX, and schema-v3 [`visible_first_boundary_audit.py`](./scripts/visible_first_boundary_audit.py) for ordered contact, production-capsule escape, surface-versus-solid collision intent, global transform/contact-distance parity, non-climb traversal and modular-seam coverage. Where streets exist, [`road-and-streetscape-semantics.md`](./references/road-and-streetscape-semantics.md) adds schema-v6 resolved-scene checks for topology, road-end policy, exporter-resolved topmost render surfaces, marking/closure intersections, class-specific placement, source-owned facade roles across opposed diagonal MSAA-normalized masks, support contacts, incident closures and complete shipping-camera coverage.

QA evidence code is now explicitly toolchain-only: exporters load project adapters transiently after instantiating the exact production root, while the provenance audit rejects QA/report paths in the production dependency closure. [`build_size_audit.py`](./scripts/build_size_audit.py) can scan the exact PCK, embedded executable, directory or ZIP for forbidden resource markers, so an export filter cannot hide a referenced QA adapter.
...
All complete games and production slices with visible motion now also use the general builder-owned [`production-motion-review.template.md`](./assets/production-motion-review.template.md) and [`motion-and-animation.md`](./references/motion-and-animation.md). They require an early gameplay-size motion cell plus exact-candidate normal-speed evidence for core input response, a complete system cycle, travel/turn/contact or transfer, dense simultaneous loops, and interruption/recovery. Valid tweens, changing coordinates, slowed playback, or selected poses cannot pass sliding, generic bounce, weightless starts/stops, missed contacts, phase-locked automation, or mistimed cause and reward.
