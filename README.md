...
Fixed/high-angle 3D districts use the composable `high-angle-3d-district-complete` modifier and [`high-angle-3d-districts.md`](./references/high-angle-3d-districts.md). Its district review fails clone filler and incoherent composition. The layered [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) workflow binds every geometry/semantic report to a recursive ResourceLoader/runtime dependency-closure digest with project, exporter and preset hashes, then runs local geometry, strict contact-exemption, whole-map coverage, and [`visible_first_boundary_audit.py`](./scripts/visible_first_boundary_audit.py) ordered-contact audits. Where streets exist, [`road-and-streetscape-semantics.md`](./references/road-and-streetscape-semantics.md) adds a fail-closed resolved-scene audit for lane/junction/sidewalk/crosswalk/marking topology, approach/T-side sidewalk-curb band closure, crosswalk/road-detail priority, building/vehicle/furniture forbidden-surface footprints, visible facade/roof/trim material slots, approach-aware furniture, authored incident closures, hidden-wall reachability pockets and complete shipping-camera junction coverage.
