...
| 42 deterministic Python helpers | Project/asset audits, transformed prop/surface/ground/clearance checks, visual checks, genre-aware difficulty and system contracts, crash/commerce/cloud/safety/upgrade/fault/hardware/assistive probes, rubric composition, capture, budgets, scorecards, and build-size checks |
...
- **Fixed/high-angle 3D districts:** visible urban/terrain boundaries, block massing, landmarks/view corridors, functional story zones, modular/material direction, full-transform prop occupancy, semantic support-surface ownership, render-ground seam sampling, overhead clearance, and measured camera motion.
...
Fixed/high-angle 3D districts use the composable `high-angle-3d-district-complete` modifier and [`high-angle-3d-districts.md`](./references/high-angle-3d-districts.md). Its district review fails repeated perimeter fences, clone filler, meaningless scatter, view corridors ending in void, and random flat facade recoloring. The separate [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) contract and [`environment_integrity_audit.py`](./scripts/environment_integrity_audit.py) resolve final Godot transforms, full prop/support footprints, visible ground triangles and vertical clearance, so complete collision coverage can no longer hide penetrations, roadway nature props or substrate seams.
