...
| --- | --- |
| 64 focused production guides | Scene architecture, visual-style selection, 2D/3D/2.5D, high-angle district production and environment integrity, UI, production-craft/product approval, saves, accessibility, AI, genre/network systems, difficulty/pacing, commerce/cloud/safety, resilience/upgrades, hardware, assets, performance, and release work |
| 44 deterministic Python helpers | Project/asset audits, resolved dependency-closure provenance, local and whole-map prop/surface/ground/collider/camera checks, visual checks, genre-aware difficulty and system contracts, crash/commerce/cloud/safety/upgrade/fault/hardware/assistive probes, rubric composition, capture, budgets, scorecards, and build-size checks |
...
- **Fixed/high-angle 3D districts:** dependency-closure candidate identity, district composition, full-transform contacts, per-zone surface topology, zero-gap shipping-camera surveys, all-static-collider/render-shell hero rasters, production occluder aliases, overhead clearance, and measured camera motion.
...
Fixed/high-angle 3D districts use the composable `high-angle-3d-district-complete` modifier and [`high-angle-3d-districts.md`](./references/high-angle-3d-districts.md). Its district review fails clone filler and incoherent composition. The layered [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) workflow first binds both geometry reports to a recursive ResourceLoader/runtime dependency-closure digest with project, exporter and preset hashes, then runs local geometry and whole-map coverage audits: final transforms/contact surfaces/clearance, single-family zones and transition/fallback budgets, complete shipping-camera tile coverage, every enabled static collider against visible mass, production occluder aliases, and topmost surface/object-pair rules.
