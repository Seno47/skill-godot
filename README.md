...
| 43 deterministic Python helpers | Project/asset audits, local and whole-map prop/surface/ground/collider/camera checks, visual checks, genre-aware difficulty and system contracts, crash/commerce/cloud/safety/upgrade/fault/hardware/assistive probes, rubric composition, capture, budgets, scorecards, and build-size checks |
...
- **Fixed/high-angle 3D districts:** district composition, full-transform contacts, per-zone surface topology, zero-gap shipping-camera surveys, all-static-collider/render-shell hero rasters, production occluder aliases, overhead clearance, and measured camera motion.
...
Fixed/high-angle 3D districts use the composable `high-angle-3d-district-complete` modifier and [`high-angle-3d-districts.md`](./references/high-angle-3d-districts.md). Its district review fails clone filler and incoherent composition. The layered [`3d-environment-integrity.md`](./references/3d-environment-integrity.md) workflow runs local geometry and whole-map coverage audits: final transforms/contact surfaces/clearance, single-family zones and transition/fallback budgets, complete shipping-camera tile coverage, every enabled static collider against visible mass, production occluder aliases, and topmost surface/object-pair rules.
